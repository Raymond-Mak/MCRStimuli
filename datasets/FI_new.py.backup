...
# FI_new数据集的8个情绪类别
FI_NEW_EMOTIONS = ["amusement", "anger", "awe", "contentment", "disgust", "excitement", "fear", "sadness"]
# 建立情绪名称到整数标签的映射
CLASSNAME_TO_LABEL = {name: i for i, name in enumerate(FI_NEW_EMOTIONS)}
LABEL_TO_CLASSNAME = {i: name for i, name in enumerate(FI_NEW_EMOTIONS)}
NUM_CLASSES = len(FI_NEW_EMOTIONS)
...
class FI_new(DatasetBase):
    ...
    def __init__(self, cfg):
        ...
        print("Loading validation data (using test set)...")
        val_items = self._read_data_from_folder("test")  # 按要求使用测试集验证
    ...
    def _read_data_from_folder(self, split_name):
        ...
        for emotion_folder in emotion_folders:
            ...
            # 检查是否是有效的情绪类别
            if emotion not in FI_NEW_EMOTIONS:
    ...
    def _subsample_classes(self, train, val, test, subsample):
        ...
        if isinstance(subsample, int):
            selected_classes = FI_NEW_EMOTIONS[:subsample]
        elif isinstance(subsample, list):
            selected_classes = [cls for cls in subsample if cls in FI_NEW_EMOTIONS]
        else:
