...
# Twitter2数据集的2个情绪类别
TWITTER2_EMOTIONS = ["neg", "pos"]
# 建立情绪名称到整数标签的映射
CLASSNAME_TO_LABEL = {name: i for i, name in enumerate(TWITTER2_EMOTIONS)}
LABEL_TO_CLASSNAME = {i: name for i, name in enumerate(TWITTER2_EMOTIONS)}
NUM_CLASSES = len(TWITTER2_EMOTIONS)
...
class Twitter2(DatasetBase):
    ...
    def _validate_dataset_structure(self):
        ...
        for emotion in TWITTER2_EMOTIONS:
        ...
        print(f"Dataset validation passed. Found all {len(TWITTER2_EMOTIONS)} emotion directories.")
    ...
    def _read_data_from_directories(self):
        ...
        if not all_items:
            raise ValueError("No valid data items were loaded from Twitter2 dataset.")

        print(f"Successfully loaded {len(all_items)} items from Twitter2 dataset.")
        return all_items
    ...
    def _subsample_classes(self, train, test, subsample):
        ...
        if isinstance(subsample, int):
            selected_classes = TWITTER2_EMOTIONS[:subsample]
        elif isinstance(subsample, list):
            selected_classes = [cls for cls in subsample if cls in TWITTER2_EMOTIONS]
        else:
