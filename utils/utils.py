...
lafter_datasets = ['DescribableTextures',  'EuroSAT', 'OxfordFlowers', 'SUN397', 'UCF101', 'ImageNetR', 'ImageNetSketch',
                   'ImageNetA', 'CIFAR10_local', 'CIFAR100_local', 'ImageNet', 'Caltech101', 'FGVCAircraft', 'Emotion6','abstract', 'FI', 'FI_Probing', 'FI_new', 'Emoset', 'Twitter1','Twitter2']
...
def setup_text_training_utils(args, model):
    ...
    model = model.float()
    params = list()
    # 安全获取milestones，如果不存在则使用默认值
    milestones = getattr(args, 'milestones', [15, 25, 35])
...
def setup_lafter_training_utils(args, model):
    ...
    # 分别设置adapter和其他参数的学习率，adapter用更高学习率来加强学习
    adapter_params = []
    other_params = []
    ...
    # 降低权重衰减以缓解欠拟合
    adapter_weight_decay = 1e-4  # 显著降低adapter的权重衰减
    ...
    print(f">>> Adapter learning rate: {args.lr * 5:.6f}, weight_decay: {adapter_weight_decay:.6f}")
    print(f">>> Other parameters learning rate: {args.lr:.6f}, weight_decay: {other_weight_decay:.6f}")

    # 添加Warmup学习率调度器策略
    # 计算warmup参数
    # warmup_epochs = max(3, args.epochs // 8)  # 自适应warmup：占总训练的12-15%
    warmup_epochs = max(2, 2)
    warmup_lr = args.lr * 0.1  # warmup起始学习率

    if hasattr(args, 'milestones') and args.milestones:
        # 有milestones时：warmup + MultiStepLR衰减
        main_scheduler = optim.lr_scheduler.MultiStepLR(optimizer, args.milestones, 0.7)
        scheduler = ConstantWarmupScheduler(optimizer, main_scheduler, warmup_epochs, warmup_lr)
        print(f">>> Using MultiStepLR with Warmup:")
        print(f"    warmup_epochs={warmup_epochs}, warmup_lr={warmup_lr:.6f} -> target_lr={args.lr:.6f}")
        print(f"    milestones={args.milestones}, decay_factor=0.7")
    else:
        # 没有milestones时：warmup + 恒定学习率
        constant_scheduler = optim.lr_scheduler.ConstantLR(optimizer, factor=1.0, total_iters=args.epochs)
        scheduler = ConstantWarmupScheduler(optimizer, constant_scheduler, warmup_epochs, warmup_lr)
        print(f">>> Using Constant Learning Rate with Warmup:")
        print(f"    warmup_epochs={warmup_epochs}, warmup_lr={warmup_lr:.6f} -> target_lr={args.lr:.6f}")

    # 降低标签平滑以减少正则化
    criteria = LabelSmoothingCrossEntropy(epsilon=0.05)  # 从0.1降低到0.05
    return optimizer, scheduler, criteria
