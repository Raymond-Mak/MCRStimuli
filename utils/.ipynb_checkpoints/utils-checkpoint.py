...
lafter_datasets = ['DescribableTextures',  'EuroSAT', 'OxfordFlowers', 'SUN397', 'UCF101', 'ImageNetR', 'ImageNetSketch',
                   'ImageNetA', 'CIFAR10_local', 'CIFAR100_local', 'ImageNet', 'Caltech101', 'FGVCAircraft', 'Emotion6','abstract', 'FI']
...
def setup_text_training_utils(args, model):
    ...
    mile_stones = getattr(args, 'milestones', [15, 25, 35])
...
def setup_lafter_training_utils(args, model):
    ...
    print(f">>> Adapter learning rate: {args.lr:.6f}, weight_decay: {adapter_weight_decay:.6f}")
    print(f">>> Other parameters learning rate: {args.lr:.6f}, weight_decay: {other_weight_decay:.6f}")

    # 添加Warmup学习率调度器策略
    # 计算warmup参数
    warmup_epochs = max(2, args.epochs // 8)  # 自适应warmup：占总训练的12-15%
